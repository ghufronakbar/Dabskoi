...
import { GetKoiSellResponse } from "@/types/api/koi";
import { Plus, Search, Trash2, Eye, FileIcon } from "lucide-react";
...
export default function KoiSells() {
  const [kois, setKois] = useState<GetKoiSellResponse[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedKoi, setSelectedKoi] = useState<GetKoiSellResponse | null>(
    null
  );
  ...
  const fetchKois = async () => {
    try {
      const response = await serviceKoi.getSells();
      setKois(Array.isArray(response) ? response : []);
    } catch (error) {
      console.error("Error fetching kois:", error);
      toast.error("Gagal mengambil daftar koi");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Yakin ingin menghapus listing koi ini?")) return;

    try {
      await serviceKoi.deleteSell(id);
      toast.success("Listing koi berhasil dihapus");
      fetchKois();
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    } catch (error: any) {
      console.error("Error deleting koi:", error);
      toast.error(
        error?.response?.data?.responseMessage || "Gagal menghapus koi"
      );
    }
  };
  ...
  const onSubmitAdd = async () => {
    try {
      ...
      const payload: Parameters<typeof serviceKoi.createSell>[0] = {
        name: name.trim(),
        type: type.trim(),
        gender,
      ...
      await serviceKoi.createSell(payload);
      toast.success("Listing koi berhasil dibuat");
      setOpenAdd(false);
      resetForm();
      fetchKois();
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      console.error(e);
      toast.error(
        e?.response?.data?.responseMessage || "Gagal membuat listing"
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-muted-foreground">
              Kelola listing koi untuk penjualan langsung
            </p>
          </div>
          ...
          >
            ...
            <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>Tambah Koi Baru</DialogTitle>
                <DialogDescription>
                  Isi detail untuk membuat listing penjualan koi.
                </DialogDescription>
              </DialogHeader>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="cth. Kohaku Premium"
                  />
                </div>
                ...
                <div className="md:col-span-2 space-y-2">
                  ...
                  <Textarea
                    ...
                    placeholder="Deskripsi singkat tentang koi..."
                    rows={4}
                  />
                </div>
                ...
                <div className="md:col-span-2 space-y-2">
                  ...
                  <FileUpload
                    value={certificate}
                    onChange={(url) => setCertificate(url)}
        ...
        <Card>
          <CardHeader>
            <CardTitle>Daftar Koi</CardTitle>
            <CardDescription>
              Semua koi yang tersedia untuk pembelian langsung
            </CardDescription>
            <div className="flex items-center space-x-2">
              <Search className="h-4 w-4 text-muted-foreground" />
              <Input
                placeholder="Cari koi berdasarkan nama atau tipe..."
                value={searchTerm}
